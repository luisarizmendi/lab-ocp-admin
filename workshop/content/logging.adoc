...
----
cat <<EOF > openshift_logging_namespace.yaml
apiVersion: v1
kind: Namespace
metadata:
...
1. Install the Elasticsearch Operator:
  ...
  b. Choose `Elasticsearch Operator` from the list of available Operators,
    and click `Install`.
  c. On the `Create Operator Subscription` page, *select Update Channel 4.2*, leave all other defaults
...
====
  ...
  c. On the `Create Operator Subscription` page, Under ensure `Installation
     ...
     `openshift-logging`. In addition, *select Update Channel 4.2* and leave all other defaults
     and then click `Subscribe`.
